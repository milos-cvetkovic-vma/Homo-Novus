...
from subscriptions import NO_SYMBOL_SUBS, DEFAULT_SUBS
...
class BitMEXWebsocket:
    ...
    def __init__(self, endpoint, symbol, callback, api_key=None, api_secret=None, subscriptions=DEFAULT_SUBS):
        ...
        self.__wait_for_symbol(symbol, subscriptions)
        if api_key:
            self.__wait_for_account(subscriptions)
    ...
    def get_ticker(self):
        ...
        if 'instrument' in self.data and self.data['instrument']:
            instrument = self.data['instrument'][0]
    ...
    def get_ticker_both_round_and_not(self):
        '''Return a ticker object. Generated from quote and trade.'''
        lastQuote = self.data['quote'][-1]
        #lastTrade = self.data['trade'][-1]
        ticker = {
        ...
        # The instrument has a tickSize. Use it to round values.
        instrument = self.data['instrument'][0]
        return {'round': {k: round(float(v or 0), instrument['tickLog']) for k, v in ticker.items()}, 'not_round': ticker}
    ...
    def __connect(self, wsURL, symbol):
        ...
        def run_forever_wrapper(ws):
            try:
                ws.run_forever(ping_interval=60, ping_timeout=20)
            except:
                self.logger.error("Run_forever_crashed")
                self.logger.flush()
                self.logger.error(traceback.format_exc())
                self.logger.flush()
                ws.sock = None

        self.wst = threading.Thread(target=lambda: run_forever_wrapper(self.ws))
    ...
    def __wait_for_account(self, subscriptions):
        '''On subscribe, this data will come down. Wait for it.'''
        # Wait for the keys to show up from the ws
        wait_for_set = {'margin', 'position', 'order', 'orderBookL2'}
        filtered = [x for x in wait_for_set if x in subscriptions]
        while not set(filtered) <= set(self.data):
            sleep(0.1)

    def __wait_for_symbol(self, symbol, subscriptions):
        '''On subscribe, this data will come down. Wait for it.'''
        wait_for_set = {'instrument', 'trade', 'quote'}
        filtered = [x for x in wait_for_set if x in subscriptions]
        while not set(filtered) <= set(self.data):
            sleep(0.1)
                ...
                if action == 'partial':
                    ...
                    if table == "execution":
                        for item in message['data']:
                            if 'ordStatus' in item and (item['ordStatus'] == "Filled" or item['ordStatus'] == "PartiallyFilled"):
                                self.callback(item)
