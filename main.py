...
conn = sqlite3.connect(
    "stocks.db", check_same_thread=False
)  # TODO: switch to in-memory db, no need to recover after program crash, too complicated at this point
c = (
    conn.cursor()
)  # TODO: this name is in conflict with local name? Maybe it can cause race-conditions.
...
class AlgoThread(threading.Thread):
            ...
            try:
                if retried:
                    ...
                    self.client = bitmex.bitmex(
                        test=g_is_test, api_key=g_api_key, api_secret=g_api_secret
                    )
        ...
        try:
            ...
            while True:  # TODO: while not stopped
                if self.reset_day:
                    ...
                    current_day1 = datetime.datetime.combine(
                    ...
                    try:
                        ...
                        ws = BitMEXWebsocket(
                            endpoint=(
                                ...
                                if not g_is_test
                                else endpoints["testnet"]
                            ),
                            symbol=g_symbol,
                            callback=self.on_update,
                            api_key=g_api_key,
                            api_secret=g_api_secret,
                ...
                if (
                    new_time - previous_refresh_time
                ) >= time_to_refresh_in_seconds or self.client == None:
                    previous_refresh_time = time.time()
                    self.tradingStatusMutex.acquire()
                    self.logger.info("Connecting")
                    try:
                ...
                assert (
                    len(active_orders) <= 1
                )  # TODO: do not support multiple active orders for now. We will stop all trading if single active order has penalties.
                for order in active_orders:
                    ...
                    if (
                        tradingStatus.sellStop
                        or tradingStatus.buyStop
                        or (
                            config_continue_next_hour
                            and (tradingStatus.r1Manual or tradingStatus.g1Manual)
                        )
                    ):
                        ...
                        try:
                            if (
                                ...
                                or (
                                    config_continue_next_hour
                                    and (
                                        tradingStatus.r1Manual or tradingStatus.g1Manual
                                    )
                                )
                            ):
                                # TODO: do retriable submit for all self.client. calls.
                                res = self.retriable_submit(
                                    lambda: self.client.Quote.Quote_get(
                                        symbol=symbol, reverse=True, count=1
                                    ).result()
                                )
                                ...
                                if tradingStatus.buyStop:
                                    self.logger.info(
                                        "Buy stop: "
                                        + str(tradingStatus.buyStop["stopPx"])
                                        + " for "
                                        + str(tradingStatus.buyStop["orderQty"])
                                    )
                                    if tradingStatus.buyStop["stopPx"] <= askPrice:
                                        ...
                                        self.r2Filled(
                                            tradingStatus, tradingStatus.currentPosition
                                        )
                                        tradingStatus.buyStop = None
                                elif tradingStatus.sellStop:
                                    self.logger.info(
                                        "Sell stop: "
                                        + str(tradingStatus.sellStop["stopPx"])
                                        + " for "
                                        + str(tradingStatus.sellStop["orderQty"])
                                    )
                                    if tradingStatus.sellStop["stopPx"] >= bidPrice:
                                        ...
                                        self.g2Filled(
                                            tradingStatus,
                                            -tradingStatus.currentPosition,
                                        )
                                        tradingStatus.sellStop = None
                                elif config_continue_next_hour and (
                                    tradingStatus.r1Manual or tradingStatus.g1Manual
                                ):  # and tradingStatus.currentPosition != 0:
                                    self.logger.info(
                                        "Current position: "
                                        + str(tradingStatus.currentPosition)
                                        + ", r1Manual: "
                                        + str(tradingStatus.g1Manual)
                                    )
                                    # Check for g1.
                                    if (
                                        tradingStatus.g1Manual
                                        and mid <= tradingStatus.g1Price
                                    ):
                                        tradingStatus.g1Manual = False
                                        self.g1Filled(tradingStatus)
                                    # Check for r1.
                                    elif (
                                        tradingStatus.r1Manual
                                        and mid >= tradingStatus.r1Price
                                    ):
                    ...
                    def getPrices(i, quantity, tradingStatus, percs):
                        ...
                        orderBuy.price = currentPrice - (
                            currentPrice * percs_dict["g1"] / 100
                        )
                        ...
                        orderSell.price = currentPrice + (
                            currentPrice * percs_dict["r1"] / 100
                        )
                    ...
                    for hour in reversed(hours):
                        ...
                        end_t = current_day + datetime.timedelta(
                            hours=hour_i + duration
                        )
                        ...
                        if tradingStatus.cycle >= currentIterationCycle + 2:
                                    ...
                                    if tradingStatus.cycle >= currentIterationCycle + 1:
                                        ...
                                        for oid in [
                                            ...
                                            try:
                                                if oid is not None:
                                                    res = (
                                                        self.client.Order.Order_cancel(
                                                            orderID=oid
                                                        ).result()
                                                    )
                                                    assert res[1].status_code == 200
                                        ...
                                        # else:
                                        if (
                                            config_consecutive_penalty_points
                                            and not tradingStatus.penalty_active
                                        ):
                                            self.resetPenaltyPoints(tradingStatus)

                                        if tradingStatus.currentPosition != 0 and (
                                            not config_continue_next_hour
                                            or last_hour_of_the_day
                                        ):
                                            self.logger.info("Placing/closing orders")
                                            res = self.client.Order.Order_new(
                                                symbol=symbol,
                                                orderQty=tradingStatus.currentPosition,
                                                ordType="Market",
                                            ).result()
                                            assert res[1].status_code == 200
                                        ...
                                        if (
                                            config_continue_next_hour
                                            and not last_hour_of_the_day
                                        ):
                                            tradingStatus.currentPosition = (
                                                prev_position
                                            )
                                        self.symbolToTradingStatus[symbol] = (
                                            tradingStatus
                                        )
                                finally:
                                    self.tradingStatusMutex.release()
                            ...
                            tradingStatus.cycle >= currentIterationCycle
                            and current_time >= hour_t
                        ):  # TODO: handle making order inactive, after all cycles.
                            ...
                            res = self.client.Quote.Quote_get(
                                symbol=g_symbol, reverse=True, count=1
                            ).result()
                            assert res[1].status_code == 200
                            resj = json.loads(res[1].text)
                            quote = resj[-1]
                            ...
                            self.logger.info(
                                "Trading status: "
                                + str(json.dumps(tradingStatus.__dict__))
                            )
                            ...
                            (orderBuy, orderSell) = getPrices(
                                i, quantity, tradingStatus, percs
                            )
                            tradingStatus.cycle += 1
                            ...
                            try:
                                ...
                                price = (
                                    tradingStatus.basePrice
                                    - (
                                        tradingStatus.basePrice
                                        * tradingStatus.percs_dict["g2"]
                                        / 100
                                    )
                                    + PRICE_FIX
                                )
                                ...
                                if (
                                    not config_continue_next_hour
                                    or tradingStatus.currentPosition == 0
                                ):
                                    self.logger.info(
                                        "Placing orders. g1: "
                                        + str(orderBuy.price)
                                    ...
                                    res = self.client.Order.Order_new(
                                        symbol=symbol,
                                        orderQty=orderBuy.totalQuantity,
                                        price=orderBuy.price + PRICE_CORRECTION,
                                        ordType="Limit",
                                    ).result()
                                    assert res[1].status_code == 200
                                    resj = json.loads(res[1].text)
                                    ...
                                    res = self.client.Order.Order_new(
                                        symbol=symbol,
                                        orderQty=-orderSell.totalQuantity,
                                        price=orderSell.price - PRICE_CORRECTION,
                                        ordType="Limit",
                                    ).result()
                                    assert res[1].status_code == 200
                                    resj = json.loads(res[1].text)
    ...
    def on_update(self, item):
        ...
        try:
            ...
            if (
                item["ordStatus"] == "Filled" or item["ordStatus"] == "PartiallyFilled"
            ) and lastQty:
                # cumQty = item['cumQty']
                ...
                if item["orderID"] == tradingStatus.r1Id:
                    ...
                    if tradingStatus.r1Id:
                        ...
                        res = self.client.Order.Order_cancel(
                            orderID=tradingStatus.r1Id
                        ).result()
                        assert res[1].status_code == 200
                    ...
                    price = tradingStatus.basePrice + (
                        tradingStatus.basePrice * tradingStatus.percs_dict["r1"] / 100
                    )
                    price = round_to(price, rounding_constant)
                    ...
                    if tradingStatus.currentPosition <= 0:
                        if not tradingStatus.r4Id:
                            ...
                            res = self.client.Order.Order_amend(
                                orderID=tradingStatus.r4Id, orderQty=quantity
                            ).result()
                            assert res[1].status_code == 200
                    elif tradingStatus.currentPosition > 0:
                        # Here we have to put buy stop order.
                        tradingStatus.buyStop["orderQty"] = (
                            tradingStatus.currentPosition
                        )
                    ...
                    price = tradingStatus.basePrice - (
                        tradingStatus.basePrice * tradingStatus.percs_dict["g1"] / 100
                    )
                    price = round_to(price, rounding_constant)
                    quantity = tradingStatus.currentPosition
                    if tradingStatus.currentPosition >= 0:
                        if not tradingStatus.g4Id:
                            ...
                            res = self.client.Order.Order_amend(
                                orderID=tradingStatus.g4Id, orderQty=quantity
                            ).result()
                            assert res[1].status_code == 200
                    elif tradingStatus.currentPosition < 0:
                        # Here we have to put sell stop order.
                        tradingStatus.sellStop["orderQty"] = (
                            tradingStatus.currentPosition
                        )
    ...
    def r1Filled(self, tradingStatus):
        ...
        if tradingStatus.currentPosition >= 0:
            ...
            if tradingStatus.currentPosition > 0:
                ...
                price = (
                    tradingStatus.basePrice
                    + (tradingStatus.basePrice * tradingStatus.percs_dict["r2"] / 100)
                    - PRICE_FIX
                )
                price = round_to(price, rounding_constant)
                tradingStatus.buyStop = dict(
                    symbol=symbol, orderQty=tradingStatus.currentPosition, stopPx=price
                )
                self.logger.info("r2 finish")
                ...
                if not tradingStatus.r3Id:
                    price = tradingStatus.r1Price - (
                        tradingStatus.r1Price * tradingStatus.percs_dict["r3"] / 100
                    )
                    price = round_to(price, rounding_constant)
                    res = self.client.Order.Order_new(
                        symbol=symbol,
                    ...
                    res = self.client.Order.Order_amend(
                        orderID=tradingStatus.r3Id, orderQty=quantity
                    ).result()
                    assert res[1].status_code == 200
            ...
            if tradingStatus.g1Id:
                res = self.client.Order.Order_amend(
                    orderID=tradingStatus.g1Id,
                    orderQty=tradingStatus.quantity + tradingStatus.currentPosition,
                ).result()
                assert res[1].status_code == 200
    ...
    def g1Filled(self, tradingStatus):
        ...
        if tradingStatus.currentPosition <= 0:
            ...
            if tradingStatus.currentPosition < 0:
                ...
                price = (
                    tradingStatus.basePrice
                    - (tradingStatus.basePrice * tradingStatus.percs_dict["g2"] / 100)
                    + PRICE_FIX
                )
                price = round_to(price, rounding_constant)
                tradingStatus.sellStop = dict(
                ...
                if not tradingStatus.g3Id:
                    price = tradingStatus.g1Price + (
                        tradingStatus.g1Price * tradingStatus.percs_dict["g3"] / 100
                    )
                    price = round_to(price, rounding_constant)
                    res = self.client.Order.Order_new(
                        symbol=symbol,
                    ...
                    res = self.client.Order.Order_amend(
                        orderID=tradingStatus.g3Id, orderQty=quantity
                    ).result()
                    assert res[1].status_code == 200
    ...
    def putG2Safety(self, stop_price, tradingStatus):
        ...
        res = self.client.Order.Order_new(
            symbol=symbol,
            orderQty=-tradingStatus.quantity,
            stopPx=backupPrice,
            ordType="Stop",
        ).result()
        assert res[1].status_code == 200
    ...
    def cancelSafeties(self, tradingStatus):
        if tradingStatus.r2BackupId:
            ...
            res = self.client.Order.Order_cancel(
                orderID=tradingStatus.r2BackupId
            ).result()
            assert res[1].status_code == 200
            tradingStatus.r2BackupId = None
        ...
        if tradingStatus.g2BackupId:
            ...
            res = self.client.Order.Order_cancel(
                orderID=tradingStatus.g2BackupId
            ).result()
            tradingStatus.g2BackupId = None
            assert res[1].status_code == 200
    ...
    def addPenaltyPoint(self, tradingStatus):
        ...
        c = conn.cursor()
        c.execute(
            """UPDATE orders SET penalty_points = penalty_points+1 WHERE symbol = ?""",
    ...
    def resetPenaltyPoints(self, tradingStatus):
        ...
        c = conn.cursor()
        c.execute(
            """UPDATE orders SET penalty_points = 0 WHERE symbol = ?""",
            (tradingStatus.symbol,),
        )
        conn.commit()
